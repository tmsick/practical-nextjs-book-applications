...
export function renderLink(flag: boolean, renderer: (attr?: { "aria-current": "page" }) => ReactNode) {
  return renderer(flag ? { "aria-current": "page" as const } : undefined)
}

export function Navigation({ children, linkClassName, currentPathname }: Props) {
  return (

...
async function getCategoryFromProps({ params }: Props) {
  const categoryName = typeof params.segments[0] === "string" ? params.segments[0] : "1"
  const page = typeof params.segments[1] === "string" ? params.segments[1] : "1"
    ...
    <div className={styles.page}>
      <Section>
        ...
        {data.category.photos.length > 0 && (
          <CardContainer>
            <PhotoIdsContainer photoIds={data.category.photos.map(photo => photo.id)}>
              {data.category.photos.map(photo => (
                <Link key={photo.id} href={`/photos/${photo.id}/view`} prefetch={true}>
                  <PhotoCard {...photo} />
                </Link>
              ))}
            </PhotoIdsContainer>

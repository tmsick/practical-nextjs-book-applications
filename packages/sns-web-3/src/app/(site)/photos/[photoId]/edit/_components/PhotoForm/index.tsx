...
function PhotoFormTitle({ defaultValue, error }: { defaultValue: string; error?: string }) {
  ...
  return (
    <Section>
      ...
      <TextField id={id} className={styles.title} name="title" defaultValue={defaultValue} />
...
function PhotoCategory({
  ...
  return (
    <Section>
      ...
      <Select id={id} className={styles.category} name="categoryId" defaultValue={categoryId}>
...
function PhotoDescription({ defaultValue, error }: { defaultValue: string; error?: string }) {
  ...
  return (
    <Section>
      ...
      <TextArea id={id} className={styles.description} name="description" defaultValue={defaultValue} />
      {error && <AlertText>{error}</AlertText>}
    </Section>
  )
}
...
export function PhotoForm({ photo, categories }: Props) {
  ...
  const [formState, formDispatch] = useFormState(updatePhoto, initialFormState({ ...photo }))
  // 【2】Client で発生したバリデーションエラーを保持
  const [clientErrors, setClientErrors] = useState<FieldErrors | undefined>(formState.error?.fieldErrors)
  // 【3】項目別に表示するエラー
    ...
    <form action={formDispatch} onSubmit={handleSubmit}>
      ...
      <div className={styles.content}>
        <div>
          ...
          />
          <PhotoCategory categoryId={photo.categoryId} categories={categories} />
        </div>
        <div>
          <PhotoDescription

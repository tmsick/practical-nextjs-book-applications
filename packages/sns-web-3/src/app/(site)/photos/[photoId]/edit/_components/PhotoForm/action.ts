...
async function getNextPrevCategoryName(prevCategoryId: string, nextCategoryId: string) {
  const { categories } = await getCategories()
  const prevCategoryName = categories.find(category => category.id === prevCategoryId)?.name
...
export async function updatePhoto(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    const { prevCategoryName, nextCategoryName } = await getNextPrevCategoryName(

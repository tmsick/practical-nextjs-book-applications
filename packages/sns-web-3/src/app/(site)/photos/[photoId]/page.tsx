...
export default async function Page({ params }: Props) {
  ...
  const [session, { photo }] = await Promise.all([getServerSession(), getPhoto({ id: params.photoId })])
  if (!photo) {
    notFound()
  }
  ...
  const [author, { category }, { liked }] = await Promise.all([
    ...
    ...(session?.user.id ? [getPhotoLike({ userId: session.user.id, photoId: photo.id })] : [{ liked: false }]),
  ])
  if (!author) {
    notFound()
  }
  ...
  return (
    <>
      ...
      <div className={styles.content}>
        <PhotoMeta photo={photo} category={category} author={author} isOwner={isOwner} />

...
function useOptimisticUpdate({
  ...
  const [optimisticComments, addOptimisticComment] = useOptimistic(comments, (prevComments, newComment: Comment) => {
...
export function ClientPhotoComment({ photoId, userId, authors, defaultComments }: Props) {
    ...
    <div>
      {userId && (
        <Section>
          ...
          <CommentForm photoId={photoId} formRef={formRef} formAction={formAction} />
        </Section>
      )}
      <Section>
        <HeadGroup>
        ...
        <CommentList comments={optimisticComments.slice(0, 10)} authors={authors} />
      </Section>
    </div>
  )
}

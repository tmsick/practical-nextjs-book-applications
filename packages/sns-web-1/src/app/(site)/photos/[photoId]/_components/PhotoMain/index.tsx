...
export async function PhotoMain({ photo, category, author, isOwner, isLoggedIn }: Props) {
  const { comments } = await getPhotoComments(photo.id)
  const authors = users
  const authorProfile = profiles.find(profile => profile.userId === author.id)
  return (
    <div className={styles.content}>
      <div className={styles.meta}>
        ...
        <Section>
          ...
          <LinkTag size="large" color="white" linkProps={{ href: `/categories/${category.name}` }}>
            {category.label}
          </LinkTag>
        </Section>
        {!isOwner && authorProfile && (
          <Section>
            ...
            <Link className={styles.user} href={`/users/${authorProfile.screenName}`}>
              <Account name={author.name} screenName={authorProfile.screenName} imageUrl={author.image} />
            </Link>
          </Section>
        )}
        {isOwner && (
          <Section className={styles.ownerActions}>
            <LinkButton href={`/photos/${photo.id}/edit`}>投稿を編集する</LinkButton>
            <DeleteButtonContainer id={photo.id}>写真を削除する</DeleteButtonContainer>
          </Section>
        )}
      </div>
